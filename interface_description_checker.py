...
def main():
    ...
    for target_device in device_list:
        try:
            ...
            best_match = guess_device_type({"device_type": "autodetect", "host": target_device, "username": target_username,
                "password": target_password})
            if best_match is None:
                continue

            print(f"\nConnecting to device: {target_device}, type: {best_match}")
            device = ConnectHandler(device_type=best_match, host=target_device, username=target_username,
                password=target_password)
        except(NetMikoAuthenticationException):
                ...
                for line in lines:
                    ...
                    if not re.search(r"master", line) and words:
                        ...
                        if len(words) > 0:
                            ...
                            int_description = int_description.group(1).rstrip() if int_description else ""
                            if not description.upper() in int_description.upper():
                                config.append(f"set interface {local} description {description}")
                            # Add newly found devices to the list
                            if ((not hostname in device_list) and (not hostname.upper() in device_list) and
                                (not hostname.lower() in device_list)):
                                device_list.append(hostname)
