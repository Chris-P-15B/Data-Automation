...
v1.3 - added device auto-detection, added Arista support
v1.2 - fixed edge case for empty show output
v1.1 - code tidying
    ...
    try:
        ...
        # Arista EOS
            ...
            for cli_line in cli_output:
                ...
                results_list.append(
                    [remote_device["host"], cli_items[3], int_description, cli_items[0]]
                )
            device.disconnect()
...
def main():
    ...
    for switch in switch_list:
        worker = Thread(
            target=find_mac_address, args=(switch, mac_address, result_list)
        )
        worker.start()
