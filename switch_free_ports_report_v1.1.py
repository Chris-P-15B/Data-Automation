...
# Checks a Cisco switch for interfaces that are currently not connected & reports relevant information, optionally output to CSV
...
if __name__ == "__main__":
    ...
    try:
        ...
        cli_output = device.send_command("show interface status | inc notconnect")
        if cli_output == None or len(cli_output) == 0:
            print(target_switch + " has no interfaces not connected.")
            sys.exit(0)
        ...
        for cli_line in cli_output:
            ...
            if re.search(r"Last input [0-9:a-z]+, ", cli_output2):
            ...
            if cli_items[-2] == "No" and (cli_items[-1] == "XCVR" or cli_items[-1] == "Transceiver" or cli_items[-1] == "Connector"):
                if (len(cli_items) == 6):
                    ...
                    interface_dict = {'interface': cli_items[0], 'description': "", 'VLAN': cli_items[-5], 'speed': cli_items[-3], 'duplex': cli_items[-4], 'type': "No Transceiver", 'last_input': last_input}
                    ...
                    for item in cli_items:
                        if item != cli_items[0]:
                            if item != "notconnect":
                                ...
                                break
                    interface_dict = {'interface': cli_items[0], 'description': int_description.rstrip(), 'VLAN': cli_items[-5], 'speed': cli_items[-3], 'duplex': cli_items[-4], 'type': "No Transceiver", 'last_input': last_input}
                    interface_list.append(interface_dict)
            # Handle regular interfaces
            else:
                if (len(cli_items) == 6):
                    # Handle no description
                    interface_dict = {'interface': cli_items[0], 'description': "", 'VLAN': cli_items[-4], 'speed': cli_items[-2], 'duplex': cli_items[-3], 'type': cli_items[-1], 'last_input': last_input}
                    interface_list.append(interface_dict)
                else:
                    # Handle descriptions with spaces
                    int_description = ""
                    for item in cli_items:
                        if item != cli_items[0]:
                            if item != "notconnect":
                                ...
                                break
                    interface_dict = {'interface': cli_items[0], 'description': int_description.rstrip(), 'VLAN': cli_items[-4], 'speed': cli_items[-2], 'duplex': cli_items[-3], 'type': cli_items[-1], 'last_input': last_input}
                    interface_list.append(interface_dict)
        ...
        if len(sys.argv) == 2:
            ...
            try:
                with open(sys.argv[1], 'w', newline='') as csv_file:
                    ...
                    result_list = [["Switch Uptime", "Interface", "Description", "VLAN", "Speed", "Duplex", "Type", "Last Input"]]
                    for interface in interface_list:
                        result_list.append([target_switch + " " + switch_uptime, interface['interface'], interface['description'], interface['VLAN'], interface['speed'], interface['duplex'], interface['type'], interface['last_input']])
                    writer.writerows(result_list)
            except OSError:
                print("Unable to write CSV file " + sys.argv[1] + ".")
                sys.exit(1)
        else:
            # Output to CLI
            print(target_switch + " " + switch_uptime + "\n\nUnused Interface List:")
            for interface in interface_list:
                print("Interface: " + interface['interface'] + ", Description: " + interface['description'] + ", VLAN: " + interface['VLAN'] + ", Speed: " + interface['speed'] + ", Duplex: " + interface['duplex'] + ", Type: " + interface['type'] + ", Last Input: " + interface['last_input'])
